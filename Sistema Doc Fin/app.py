...
<body>
  <main class="shell">
    <section class="hero">
      ...
      <p class="lede">Envie uma pasta inteira ou um lote grande de PDFs, rode a lógica de ordenação e baixe o PDF final já organizado. Esta versão está preparada para lotes grandes, inclusive acima de 500 MB quando a hospedagem tiver memória e disco suficientes.</p>
    </section>
  ...
  <script>
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const totalBytes = chosenFiles.reduce((sum, file) => sum + file.size, 0);
      const totalMB = totalBytes / (1024 * 1024);
      if (totalMB > MAX_UPLOAD_MB) {
        status.textContent = `O lote tem ${formatMB(totalBytes)} MB, acima do limite configurado de ${MAX_UPLOAD_MB} MB.`;
        summary.innerHTML = `<div class="bad">Reduza o lote ou aumente MAX_UPLOAD_MB no servidor.</div>`;
        return;
      }
      ...
      status.textContent = `Enviando e processando ${chosenFiles.length} PDFs (${formatMB(totalBytes)} MB)...`;
      ...
      try {
        ...
        try {
          ...
          const preview = responseText ? responseText.slice(0, 500) : "resposta vazia";
          throw new Error(`O servidor não devolveu JSON. Código HTTP ${response.status}. Resposta recebida: ${preview}`);
        }
        if (!response.ok) {
          throw new Error(payload.error || `Falha ao processar os arquivos. Código HTTP ${response.status}.`);
        }
...
class PDFCombinerHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            try:
                build_json_response(self, {"error": f"Falha grave no servidor: {exc}"}, status=500)
            except Exception:
                pass
